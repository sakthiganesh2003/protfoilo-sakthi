  ...
  return (
    <header className="fixed w-full top-0 z-50 bg-slate-950/80 backdrop-blur-xl border-b border-slate-800/50 shadow-lg shadow-black/20 transition-all duration-300">
      <nav className="container mx-auto px-6 flex items-center justify-between h-20">
        ...
        <Link 
          href="/" 
          className="font-extrabold text-2xl tracking-tighter bg-gradient-to-r from-violet-400 via-cyan-400 to-pink-400 bg-clip-text text-transparent hover:opacity-80 transition-opacity"
        >
          SakthiGanesh
        </Link>
        ...
        <div className="hidden md:flex space-x-10 items-center">
          {['About', 'Skills', 'Projects', 'Contact'].map((item) => (
            ...
            >
              {item}
              <span className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-gradient-to-r from-violet-500 to-cyan-500 transition-all duration-300 group-hover:w-full rounded-full"></span>
            </a>
          ))}
